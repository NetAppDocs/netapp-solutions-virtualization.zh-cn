...
---
= 使用 Trident Protect 保护 Red Hat OpenShift Virtualization 中的虚拟机
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
OpenShift Virtualization VM 的持久磁盘可以由使用 link:https://docs.netapp.com/us-en/trident/["Trident犯罪现场调查"] 集成到 OpenShift 集群的 ONTAP 存储提供支持。在本节中，我们使用 link:https://docs.netapp.com/us-en/trident/trident-protect/learn-about-trident-protect.html["Trident Protect"] 创建 VM 的快照和备份，包括其数据卷到 ONTAP 对象存储。

然后，我们在需要时从快照或备份中恢复。

Trident Protect 可对 OpenShift 群集上的应用程序和虚拟机执行快照、备份、还原和灾难恢复。对于 OpenShift Virtualization 虚拟机，可使用 Trident Protect 保护的数据包括与虚拟机关联的 Kubernetes 资源对象、持久卷和内部映像。
...
[%collapsible]
====
在为应用程序或 VM 创建快照和备份之前，必须在 Trident Protect 中配置对象存储以存储快照和备份。这是使用 bucket CR 完成的。只有管理员可以创建 bucket CR 并进行配置。bucket CR 在 Trident Protect 中称为 AppVault。AppVault 对象是存储桶的声明性 Kubernetes 工作流表示。AppVault CR 包含在保护操作中使用存储桶所需的配置，例如备份、快照、还原操作和 SnapMirror 复制。
...
此单文件还原方法仅适用于在客户端 Pod 中作为文件系统装载的 NAS/基于文件的卷，例如 NFS 卷。它不适用于 SAN/原始块卷，包括如前所示使用 `volumeMode: Block`配置的虚拟机磁盘。

对于受支持的基于 NAS/文件的卷，恢复特定文件的简单方法是使备份卷上的快照目录对客户端可见。这可以通过编辑 Trident 后端配置来实现，具体方法是包含 defaults 部分并将 `snapshotDir`参数设置为 `true`。这样即可从与主卷相同的挂载位置访问 `.snapshot`目录。 `.snapshot`目录包含所有快照。您可以切换到所需的快照目录，找到要还原的文件，然后将其复制到所需位置。


NOTE: 此方法会向具有挂载访问权限的工作负载/用户公开历史数据，并且可能会违反最小特权/合规性预期。请考虑安全/运营影响，并注意您应该临时启用它，并在恢复文件后将其恢复。此外，如果您使用的是 NFS 第 4 版，则在使用  `ls` 命令列出文件时，将无法看到  `.snapshot` 目录。但是，您可以  `cd` 到  `.snapshot` 目录。

.通过访问 .snapshot 目录并从快照复制文件进行单个文件还原的示例
[%collapsible]
====
[source, yaml]
----
...
----
image:rh-os-n-use-case-ocpv-tp-dp-sfr-001.png["VM 的 pod 和 PVC"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-002.png["虚拟机磁盘的 ONTAP 卷名称"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-003.png["快照目录在客户端中可见"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-004.png["VM 中的 mount 目录"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-005.png["丢失虚拟机中的文件并将文件从快照目录复制到主目录以还原它"]
