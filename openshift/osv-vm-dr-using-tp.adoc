...
---
= 使用 Trident Protect 为红帽 OpenShift 虚拟化中的虚拟机设置故障转移和故障恢复
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* 必须安装 Trident Protect 才能为 OpenShift 虚拟机实施故障转移和故障恢复操作。请参见此处的说明 link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-installation.html["安装 Trident Protect"]


image:redhat-openshift-ocpv-tp-001.png["OCP-v Trident Protect 安装在 trident-protect 命名空间中"]
...
本节演示如何使用 ONTAP S3 对象存储在 Trident Protect 中设置 AppVault 对象。
...
== 在新命名空间中为灾难恢复 VM 创建 Trident Protect 应用程序
