...
summary: 'VMware Cloud Foundation (VCF) 支持广泛的补充存储选项，以扩展 VCF 管理域和工作负载域上的存储。' 
---
...
[role="lead"]
VMware Cloud Foundation (VCF) 支持广泛的补充存储选项，以扩展 VCF 管理域和工作负载域上的存储。
...
您可以使用以下协议来扩展 VCF 管理域和工作负载域中的存储。为您的环境选择最佳选项，将补充存储无缝集成到您的 VCF 部署中。

.iSCSI
使用标准以太网网络的基于块的协议。非常适合需要高性能、灵活性和经济高效扩展的环境。

iSCSI 广泛用于 VMFS 数据存储，并支持高级 ONTAP 功能，例如快照和克隆。ONTAP 9.19.1 及更高版本也支持 AFF 系统上的对称主动-主动配置。最初，该功能仅在 ASA 系统上可用。
...
ONTAP 9.19.1 及更高版本也支持 AFF 系统上的对称主动-主动配置。最初，该功能仅在 ASA 系统上可用。
...
* *可扩展性：* ONTAP 的架构允许 NFS 有效扩展，支持不断增长的数据需求，而无需对基础设施进行重大更改。FlexGroup 与 pNFS 结合使用，可将 NFS 数据存储扩展到 PB 级别。
...
.NVMe-oF
通过使用 TCP/IP 的标准以太网网络提供高性能和低延迟的现代协议。NVMe-oF（NVMe over Fabrics）非常适合要求苛刻的应用程序和大规模数据操作，无需专用硬件即可提供可扩展性和成本效益。

ONTAP 9.19.1 及更高版本也支持 AFF 系统上的对称主动-主动配置。最初，该功能仅在 ASA 系统上可用。

* *高性能：*以低延迟和高数据传输速率提供卓越的性能。这对于要求苛刻的应用程序和大规模数据操作至关重要。
* *可扩展性：*支持可扩展配置，允许 IT 管理员随着数据需求的增长无缝扩展其基础设施。
* *经济高效：* 在标准以太网交换机上运行，并封装在 TCP 数据报中。无需特殊设备即可实施。NVMe-oF over TCP 是常用的。
