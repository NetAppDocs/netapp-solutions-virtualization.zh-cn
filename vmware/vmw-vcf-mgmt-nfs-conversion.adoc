...
在 vCenter 设备和 SDDC 管理器上使用 Broadcom 的 VMware VCF 导入工具来验证配置并为 vSphere 和 VCF 环境提供转换和导入服务。

在适当的情况下，可以参考 VMware 文档以获取更多说明。



== 步骤 1：复制 VCF 导入工具并运行预检查

使用 vCenter 设备上的 VCF 导入工具来验证 vSphere 集群在 VCF 转换或导入过程中是否处于健康状态。
...
+
image:../media/vmware-vcf-import-nfs-011.png["vcf导入工具预检"]
...
在您计划转换为 VCF 管理域的同一 vSphere 集群上部署 SDDC Manager 设备，然后为转换过程创建 NSX 部署规范。
...
NOTE: 在转换或导入操作中部署 NSX Manager 群集时，使用 NSX-VLAN 网络。有关 NSX-VLAN 网络限制的信息，请参阅 VMware 文档： https://techdocs.broadcom.com/fr/fr/vmware-cis/vcf/vcf-5-2-and-earlier/5-2/considerations-before-converting-or-importing-existing-vsphere-environments-into-vcf.html["将现有 vSphere 环境转换或导入 VMware Cloud Foundation 之前的注意事项"] 。



== 步骤 3：将软件上传到 SDDC 管理器
...
* 有关 VCF 导入工具的更多信息，请参阅 VMware 文档：link:https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-5-2-and-earlier/5-2/map-for-administering-vcf-5-2/importing-existing-vsphere-environments-admin/vcf-import-tool-options-and-parameters-admin.html["VCF 导入工具选项和参数"] 。
