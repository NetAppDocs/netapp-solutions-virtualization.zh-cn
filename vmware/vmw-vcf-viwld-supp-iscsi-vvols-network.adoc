...
summary: 为工作负载域中 vSphere 主机上的 iSCSI 存储配置网络。您将为 iSCSI 流量创建分布式端口组，并使用 vSphere 客户端设置 VMkernel 适配器，以实现可靠的连接和多路径。 
---
= 为 VCF 工作负载域中 vSphere 主机上的 iSCSI 配置网络
:allow-uri-read: 
...
[role="lead"]
为工作负载域中 vSphere 主机上的 iSCSI 存储配置网络。您将为 iSCSI 流量创建分布式端口组，并使用 vSphere 客户端设置 VMkernel 适配器，以实现可靠的连接和多路径。

使用结合了 vCenter 单点登录的 vSphere 客户端在工作负载域集群上执行这些步骤。同一个 vSphere 客户端可同时管理管理域和工作负载域。
...
使用 vSphere 客户端在工作负载域中的每台 vSphere 主机上执行以下步骤。
...
在为工作负载域中的所有 vSphere 主机上的 iSCSI 配置网络后，link:vmw-vcf-viwld-supp-iscsi-vvols-storage.html["为 iSCSI vVols配置存储"]。
