...
summary: 在 VMware Cloud Foundation 管理域中的 vSphere 主机上配置 iSCSI 网络，以启用与 ONTAP 存储系统的连接。您将创建具有 VLAN 分离的分布式端口组，为冗余配置上行链路绑定，并在每台 vSphere 主机上设置 VMkernel 适配器，以建立用于故障转移功能的专用 iSCSI 路径。 
---
= 在 VCF 管理域中的 vSphere 主机上为 iSCSI 配置网络
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 VMware Cloud Foundation 管理域中的 vSphere 主机上配置 iSCSI 网络，以启用与 ONTAP 存储系统的连接。您将创建具有 VLAN 分离的分布式端口组，为冗余配置上行链路绑定，并在每台 vSphere 主机上设置 VMkernel 适配器，以建立用于故障转移功能的专用 iSCSI 路径。

使用 vSphere 客户端在 VCF 管理域集群上执行以下步骤。
...
在管理域中的每个 vSphere 主机上创建 VMkernel 适配器。
...
在管理域中的所有 vSphere 主机上为 iSCSI 配置网络后，link:vmw-vcf-mgmt-supplemental-iscsi-storage.html["在 vSphere 主机上为 iSCSI 配置存储"]。
