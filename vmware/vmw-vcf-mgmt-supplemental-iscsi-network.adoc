...
summary: 在 VMware Cloud Foundation 管理域中的 ESXi 主机上配置 iSCSI 网络以实现与ONTAP存储系统的连接。您将创建具有 VLAN 分离的分布式端口组，配置上行链路组合以实现冗余，并在每个 ESXi 主机上设置 VMkernel 适配器以建立专用的 iSCSI 路径来实现故障转移功能。 
---
= 在 VCF 管理域中为 ESXi 主机配置 iSCSI 网络
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 VMware Cloud Foundation 管理域中的 ESXi 主机上配置 iSCSI 网络以实现与ONTAP存储系统的连接。您将创建具有 VLAN 分离的分布式端口组，配置上行链路组合以实现冗余，并在每个 ESXi 主机上设置 VMkernel 适配器以建立专用的 iSCSI 路径来实现故障转移功能。
...
在管理域中的每个 ESXi 主机上创建 VMkernel 适配器。
...
在工作负载域中的所有 ESXi 主机上配置 iSCSI 网络后，link:vmw-vcf-mgmt-supplemental-iscsi-storage.html["在 ESXi 主机上配置 iSCSI 存储"] 。
