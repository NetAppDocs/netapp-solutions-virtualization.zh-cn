...
summary: '在工作负载域中的 vSphere 主机上为 NVMe over TCP (NVMe/TCP) 存储配置网络。您将为 NVMe 流量创建分布式端口组，在每个 vSphere 主机上设置 VMkernel 适配器，并添加 NVMe/TCP 适配器以实现可靠的连接和多路径。' 
---
= 在 VCF 工作负载域中的 vSphere 主机上为 NVMe/TCP 配置网络
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
在工作负载域中的 vSphere 主机上为 NVMe over TCP (NVMe/TCP) 存储配置网络。您将为 NVMe 流量创建分布式端口组，在每个 vSphere 主机上设置 VMkernel 适配器，并添加 NVMe/TCP 适配器以实现可靠的连接和多路径。

使用 vSphere 客户端在工作负载域集群上执行以下步骤。在这种情况下，正在使用 vCenter Single Sign-On，因此 vSphere 客户端对于管理域和工作负载域都是通用的。



[[step-1-create-distributed-port-groups-for-nvmetcp-traffic]]
== 步骤1：为NVMe/TCP流量创建分布式端口组
...
.步骤
. 从 vSphere 客户端导航到工作负载域清单中的某台 vSphere 主机。从*配置*选项卡中选择 *VMkernel 适配器*，然后单击*添加网络……*开始。
+
.显示示例
[%collapsible]
====
...
工作负载域群集中的每台 vSphere 主机都必须为每个专用于存储流量的已建立 NVMe/TCP 网络安装 NVMe/TCP 软件适配器。

要安装 NVMe/TCP 适配器并发现 NVMe 控制器，请完成以下步骤。

. 在 vSphere 客户端中，导航到工作负载域集群中的某个 vSphere 主机。在*配置*选项卡中，单击菜单中的*存储适配器*。
