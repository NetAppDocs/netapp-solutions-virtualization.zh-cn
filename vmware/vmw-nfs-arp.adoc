...
如果所有这些都需要协调并与 SIEM 工具集成，那么可以使用像NetApp Ransomware Resilience这样的离线服务。这是一项旨在保护数据免受勒索软件侵害的服务。该服务为基于应用程序的工作负载（例如 Oracle、MySQL、VM 数据存储和本地 NFS 存储上的文件共享）提供保护。

在本例中，NFS 数据存储“Src_NFS_DS04”使用NetApp Ransomware Resilience进行保护。


NOTE: 以下步骤适用于BlueXP。工作流程与NetApp Console类似。

image:nfs-arp-005.png["该图显示输入/输出对话框或表示书面内容"]

image:nfs-arp-006.png["该图显示输入/输出对话框或表示书面内容"]

有关配置NetApp Ransomware Resilience的详细信息，请参阅：link:https://docs.netapp.com/us-en/data-services-ransomware-resilience/rp-start-setup.html["设置 NetAp 勒索软件抵御能力"]和link:https://docs.netapp.com/us-en/data-services-ransomware-resilience/rp-use-settings.html#add-amazon-web-services-as-a-backup-destination["配置 NetAp 勒索软件恢复设置"]。
...
取证分析完成后，即可使用SnapCenter或NetApp Ransomware Resilience快速无缝地完成恢复。使用SnapCenter，转到受影响的虚拟机，然后选择要还原的相应快照。

image:nfs-arp-010.png["该图显示输入/输出对话框或表示书面内容"]

本节将探讨NetApp Ransomware Resilience如何协调从勒索软件事件中恢复，其中虚拟机文件已被加密。


NOTE: 如果虚拟机由SnapCenter管理，NetApp Ransomware Resilience会使用虚拟机一致性流程将虚拟机恢复到之前的状态。

. 访问NetApp Ransomware Resilience， NetApp Ransomware Resilience仪表板上将显示警报。
. 单击警报可查看生成警报的特定卷上的事件
+
