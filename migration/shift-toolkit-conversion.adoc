...
[role="lead"]
使用 Shift Toolkit 将 VMware ESX 虚拟机磁盘 (VMDK) 转换为 Microsoft Hyper-V (VHDX) 磁盘格式或转换为 RAW/QCOW2 磁盘格式。此过程包括设置资源组、创建转换蓝图和计划转换。



== 概述

Shift Toolkit 支持虚拟机管理程序之间以下格式的虚拟磁盘的磁盘级转换，同时支持 ONTAP 上的 FlexVol 和 FlexGroup 卷：
...
转换后的 qcow2 文件与任何 KVM hypervisor 兼容。例如，可以使用 virt-manager 将 qcow2 文件与基于 RHEL 的 KVM 结合使用来创建 VM，也可以与 Ubuntu KVM、基于 Rocky Linux 的 KVM 等结合使用。在使用 NetApp Trident 导入后，同样可以与 Oracle Linux virtualization manager、Proxmox、OpenNebula、HP VME 以及 OpenShift 虚拟化结合使用。其目标是提供磁盘（在数秒至数分钟内完成转换），然后可以将其集成到组织用于配置 VM 和分配网络的现有自动化脚本中。这种方法有助于缩短整体迁移时间，其中磁盘转换由 Shift 工具包 API 处理，其余脚本用于启动 VM。

NetApp Shift Toolkit 还支持将存储在 FlexGroup 卷上的 VMware VMDK 磁盘转换为原始磁盘格式，从而实现到基于 KVM 的平台（如 Proxmox 或 OLVM）的无缝迁移。


NOTE: 使用 FlexGroup 存储时，仅支持 RAW 格式转换。此外，在执行仅 KVM 转换时，Shift 工具包会在准备过程中注入 VirtIO 驱动程序。
...
NOTE: 对于 OLVM，也可以使用 qcow2 文件格式。此项选择可在创建蓝图时进行。
...
NOTE: 要在虚拟机中使用转换后的 RAW 磁盘映像，必须通过 OpenShift 控制台或 OC 命令手动创建虚拟机（通过 YAML 为 OpenShift 创建），或者使用 OLVM REST API/UI 为 OLVM 创建，并且必须将磁盘附加到虚拟机。除此之外，还应手动映射网络。

+

NOTE: 手动创建虚拟机时，请确保指定正确的启动类型（EFI 或 BIOS）。
