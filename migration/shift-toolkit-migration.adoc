...
* link:shift-toolkit-migrate-esxi2osvmtv.html["使用虚拟化迁移工具包 (MTV) 从 VMware ESXi 迁移到 Red Hat OpenShift 虚拟化"]
...
可以直接从 UI 中删除从未执行过的蓝图。但是，在触发蓝图后，它会创建必须删除的作业历史记录，然后才能删除蓝图本身。使用 PowerShell 删除脚本清理这些作业记录，之后可以删除蓝图及其关联的资源组。

.步骤
. 通过导航到 *Settings > Developer Access > Script block*，从 Shift Toolkit UI 下载删除脚本。
+
image::shift-toolkit-082.png[根据蓝图名称删除作业]

. 解压缩 zip 文件夹。该包包含两个文件：
+
** `removeBpJobs.json` - 配置文件
** `removeBpJobs.ps1` - PowerShell 脚本


. 编辑 `removeBpJobs.json`文件以指定：
+
** 此操作的执行名称
** Shift Toolkit IP 地址
** 管理员凭据
...
脚本成功完成后，您可以从 Shift Toolkit UI 中删除蓝图及其关联的资源组。
