...
在计划的维护窗口期内，虚拟机必须正常关机。在开始迁移之前，请确认所有先决条件均已满足：

* 确保 Shift Toolkit VM 属于域。
* 确保 CIFS 共享配置了适当的权限
* 验证用于迁移或转换的 qtree 是否具有正确的安全样式
* 使用集群中任意 Hyper-V 主机上的 Hyper-V 管理器测试创建虚拟机，并将 VHDx 文件放置在 CIFS 共享上。
* 使用“程序和功能”或 PowerShell 添加 Hyper-V 管理工具，从 Shift Toolkit VM 测试创建 VM： `add-windowsfeature rsat-hyper-v-tools`
