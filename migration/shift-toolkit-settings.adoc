...
Shift 工具包现在提供了一个针对蓝图级别的验证选项，该选项在迁移状态标记为"成功"或"部分失败"后启用。成功后，验证选项将启动一个新作业，并检查蓝图映射中指定的 VM 设置。验证迁移作业的状态将在作业监控中列出，可以通过单击作业并选择"查看日志"查看详细信息。


NOTE: 在创建蓝图时也可以选择验证选项，在这种情况下，一旦迁移作业完成，验证作业将在 5 分钟后触发。


NOTE: 如果在迁移作业之后选择了验证，则会即时触发验证作业，向导将列出该作业。

image::shift-toolkit-471.png[验证迁移作业详细信息]

image::shift-toolkit-472.png[验证迁移作业列表]
...
拆分工作流程（仅适用于使用 ONTAP NAS 驱动程序的 OpenShift 迁移蓝图）会自动发现与虚拟机关联的卷及其对应的 FlexClone 卷。

根据 ONTAP 版本，自动选择适当的拆分策略。一般指引如下：

* ONTAP 9.17.1 及更高版本上的卷拆分克隆，具有 20 个并行执行
* ONTAP 版本 9.17.1 之前的卷移动，最多使用四个并行线程


拆分克隆过程会持续监控进度并相应地管理作业队列。拆分或卷移动操作完成后，将删除关联的快照。

拆分步骤作业的状态将显示在 Job Monitoring 中。要查看更多详细信息，请点击拆分作业并选择"View Logs"。


NOTE: 快照删除后，Shift Toolkit 当前不会调整卷的大小。我们计划在将来的版本中提供此功能。

image::shift-toolkit-473.png[拆分工作流策略选择]

image::shift-toolkit-474.png[拆分工作流作业进度]
...
Shift Toolkit 包括用于将迁移作业迁移到目标虚拟机管理程序的高级虚拟机管理程序设置。这些设置允许迁移管理员根据需要为每个 VM 迁移波启用功能。Shift Toolkit 在运行时读取设置，因此每个作业在启动时应用所选选项。

image::shift-toolkit-475.png[高级虚拟机监控程序设置概述]

image::shift-toolkit-476.png[高级虚拟机监控程序设置选项]

image::shift-toolkit-477.png[高级 Hypervisor 设置配置]

image::shift-toolkit-478.png[高级虚拟机监控程序设置作业详细信息]
