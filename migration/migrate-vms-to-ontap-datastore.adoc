...
NOTE: 一些使用外部管理工具（如 VCF Automation、vSphere Supervisor（或其他 Kubernetes 风格））部署的虚拟机通常取决于虚拟机存储策略。如果在同一虚拟机存储策略内的数据存储区之间迁移，则对应用程序的影响应该较小。请与应用程序所有者联系，以正确地将这些虚拟机迁移到新的数据存储区。vSphere 8 引入 https://techdocs.broadcom.com/us/en/vmware-cis/vsphere/vsphere/8-0/vcenter-and-host-management/migrating-virtual-machines-host-management/migration-with-vmotion-host-management/virtual-machine-conditions-and-limitation-for-vmotion-host-management/how-to-prepare-an-application-for-vsphere-vmotion-host-management.html["针对延迟敏感型应用程序的 vSphere vMotion 通知"]是为了让应用程序为 vMotion 做好准备。
