...
* *集成数据保护和低成本灾难恢复：*使用NetApp SnapCenter卸载虚拟工作负载备份和恢复。利用NetApp Disaster Recovery实现更低的 RPO 和更短的 RTO 数据复制时间。这样就无需昂贵的替代基础设施，为 VMware 工作负载提供经济实惠的灾难保护。无论从本地到本地NetApp ONTAP环境，还是到 VMware 云， NetApp支持的存储都能确保强大的保护。
...
* 使用ONTAP One，即可访问所有ONTAP功能和强大的安全性，而无需支付许可费用。与NetApp Disaster Recovery结合使用可进一步降低成本。
...
NetApp提供业界领先的基于 SaaS 的灾难恢复解决方案，可以显著降低成本并减少复杂性。无需购置和部署昂贵的替代基础设施。
...
使用NetApp Disaster Recovery，它集成到NetApp Console中，客户可以在其中发现其本地 VMware vCenter 以及ONTAP存储，创建资源组，创建灾难恢复计划，将其与资源组关联，并测试或执行故障转移和故障恢复。  NetApp Disaster Recovery利用 ONTAP 的FlexClone技术，从灾难恢复站点上最后一个复制的快照创建 NFS 或 VMFS 数据存储的空间高效副本。 当需要（计划内或计划外）进行实际故障转移时，只需点击几下， NetApp Disaster Recovery即可协调所有必要步骤，自动在指定的灾难恢复站点上启动受保护的虚拟机。SnapMirror提供存储级块复制，使两个站点能够通过增量更改保持最新状态，从而实现长达 5 分钟的 RPO。也可以将灾难恢复程序模拟为常规演练，而不会影响生产和复制数据存储，也不会产生额外的存储成本。NetApp Disaster Recovery 利用 ONTAP 的 FlexClone 技術，從災難復原（DR）站點上最後一次複寫的 Snapshot 建立 NFS 或 VMFS 資料存放區的空間效率複本。灾难恢复测试完成后，只需删除测试环境，不会对实际复制的生产资源造成任何影响。當需要（計劃內或非計劃性）實際容錯移轉時，只需幾個點擊，NetApp Disaster Recovery 即可協調所有必要步驟，自動在指定的災難復原站點啟動受保護的虛擬機器。该服务还会反转SnapMirror与主站点的关系，并在需要时将任何更改从辅助站点复制到主站点，以进行故障恢复操作。与其他知名替代方案相比，所有这些目标都可以以极低的成本实现。
